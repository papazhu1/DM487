...
from ..tree.tree import (WeightedDecisionTreeClassifier, 
                         ...
                         WeightedExtraTreeClassifier)
from ..utils import get_rf_tree_data
import numpy as np

from ..tree._tree import TREE_UNDEFINED
...
class RandomForestClassifierWithWeights(RandomForestClassifier):
    ...
    def n_paths(self):
        ...
        for tree in self.estimators_:
            ## :  _tree.pyx line 66 : TREE_UNDEFINED = -2 
            ## 如果feature[i] == -2 , 说明树的结点 i 是一个叶节点;
            ## 所以 out 累计 所有树的所有叶节点个数 
            ## _tree.pyx line 770, 如果是叶子节点，它的feature值为 TREE_UNDEFINED
            out += np.sum(tree.tree_.feature == TREE_UNDEFINED)  
        return out
...
class ExtraTreesClassifierWithWeights(ExtraTreesClassifier):
    @property
    def n_paths(self):
        if not hasattr(self, "estimators_"):
            return 0
        out = 0
        for tree in self.estimators_:
            ## c: in _tree.pyx line 66 : TREE_UNDEFINED = -2 
            ## 如果feature[i] == -2 , 说明树的结点 i 是一个叶节点;
            ## 所以 out 累计 所有树的所有叶节点个数 
            out += np.sum(tree.tree_.feature == TREE_UNDEFINED)  
        return out
    def fit(self, X, y, sample_weight=None, feature_weight=None):
        self.base_estimator = WeightedExtraTreeClassifier()
        self.base_estimator_ = WeightedExtraTreeClassifier()
        if feature_weight is not None:
            self.base_estimator.feature_weight = feature_weight
            self.base_estimator_.feature_weight = feature_weight
            
        return super(ExtraTreesClassifierWithWeights, self).fit(X, y, sample_weight)
