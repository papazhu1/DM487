...
cdef class ClassificationCriterion(Criterion):
    ...
    cdef int init(self, DOUBLE_t* y, SIZE_t y_stride,
        ...
        for p in range(start, end):
            ## :  samples数组第p个元素的值：i, 表示原始数据(X/y)第i行
            ## w表示第i个样本的权重
            ## k循环：对于第k个输出(multioutput)，c：第i个样本在第k个输出上的label
            ## sum_total: 对第k个输出，第c类label，记录它的权值
    ...
    cdef int reset(self) nogil except -1:
        ...
        for k in range(self.n_outputs):
            ## : “准备”计算left right节点的类别权重比例
            ## left设为0， right设为sum_total
    ...
    cdef int update(self, SIZE_t new_pos) nogil except -1:
        ...
        ## :
        ## criterion.update() 在 _splitter.pyx 中 BestSplitter.node_split()中调用
        ## 此时已经把samples[start,end] 按照特征值排好序了，即samples[start]对应的样本在这个属性上最小
        ## 如果new_pos -pos <= end-new_pos， 那只需要计算 sum_left ，即左边节点的impurity即可，因为sum_total已知
