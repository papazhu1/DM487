...
class TreeWeightsMixin(BaseDecisionTree):
    ...
    def fit(self, X, y, 
            ...
            check_input=True,
            X_idx_sorted=None):
        ## : check out BaseDecisionTree.fit(), looks identical
        ## maybe update presort to latest BaseDecisionTree's presort ( deprecated )
        ## finally : the TreeBuilder use feature_weight
        ...
        if is_classification:
            ...
            ## : 下面这一段是把原始的label数组进行编码，比如之前的取值范围可能是[11,20), 编码后取值[0,9)
            ## 即：变成第几个label，而不一定是原始的label值
        ...
        if not isinstance(criterion, Criterion):
            if is_classification:
                criterion = CRITERIA_CLF[self.criterion](self.n_outputs_,
                                                         self.n_classes_)
                ## c: call  ClassificationCriterion.__cinit__()    
        ...
        if is_classification:
            ...
            self.tree_ = Tree(self.n_features_, 
                              ...
                              self.n_outputs_)
        ## c:  Tree 主要是负责存储node 列表，表示了树的结构，只有method: _add_node 会改变树的结构
        ## node存储了 child_left, child_right, impurity, feature, threshold 信息
...
class WeightedExtraTreeClassifier(TreeWeightsMixin, ExtraTreeClassifier):
    def __init__(self,
                 criterion="gini",
                 splitter="random",
                 max_depth=None,
